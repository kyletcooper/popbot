...
<wrd-container id="popbot">
    ...
    <?php /*
    <div class="wrd-row">
        <div class="wrd-col-12 wrd-col-md-6 wrd-col-xxl-8">
            <div class="wrd-section">
                <h2>
                    <wrd-icon icon="dashboard_customize" label="Custom Templates" style="--fill: #D204B0"></wrd-icon>
                </h2>

                <?php if (!pb_templates_display_cats($categoriesCustom, true)) : ?>

                    <div class="wrd-empty">
                        <div>
                            <h3>You haven't uploaded any custom templates yet</h3>
                            <p>You can create your own PopBot templates or upload templates from the web to get even more looks.</p>
                        </div>
        ...
        <div class="wrd-col-12 wrd-col-md-6 wrd-col-xxl-4">
            <div class="wrd-section">
                <h2>
                    <wrd-icon icon="cloud_upload" label="Upload a Template" style="--fill: #D204B0"></wrd-icon>
                </h2>

                <ol>
                    <li>You can upload PHP template files to use for your PopBots.</li>
                    <li>To update a template, upload the new version with the same file name.</li>
                </ol>


                <?php action::get('popbot_uploadTemplate')->openForm(["enctype" => "multipart/form-data"]); ?>

                <label class="wrd-fileupload">
                    <input name="template[]" type="file" multiple accept=".php" onchange="this.form.submit()" />
                </label>

                <?php action::get('popbot_uploadTemplate')->closeForm(); ?>


                <?php if (array_key_exists("msg", $_GET)) : ?>

                    <div class="wrd-error">
                        <?php esc_html_e($_GET['msg']) ?>
                    </div>
