...
/**
 * Contains the Popbot_Template class.
 */

namespace popbot;

$popbot_current_template = null;

/**
 * Represents a template for a PopBot post, in either the uploads directory or stored in the plugin files.
 */
class Popbot_Template {
	...
	function enqueue_assets() {
		...
		if ( isset( $this->details['version'] ) ) {
			$ver = $this->details['version'];
		}

		if ( $this->has_css() ) {
			wp_enqueue_style( 'popbot-style-' . $this->slug, $this->get_css_url(), array(), $ver );
		}

		if ( $this->has_js() ) {
			wp_enqueue_script( 'popbot-script-' . $this->slug, $this->get_js_url(), array(), $ver, true );
		}
	}
	...
	static function get( $slug ) {
		...
		if ( ! file_exists( $template->get_dir() ) ) {
			return new \WP_Error( 'missing_template', sprintf( __( 'Template does not exist with slug: %s.', 'popbot' ), $slug ) );
		}
		...
		if ( $args['category'] ) {
			$templates = array_filter(
				...
				function ( $template ) use ( $args ) {
					$cat        = strtolower( $template->details['category'] );
					$cat_search = strtolower( $args['category'] );
					return $cat === $cat_search;
				}
			);
		}
	...
	static function create(): Popbot_Template {
	 // phpcs:ignore This function is not implemented in this version.
		// NOT IMPLEMENTED.
