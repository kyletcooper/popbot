...
/**
 * Contains the Popbot class.
 */

namespace popbot;

/**
 * Manages a PopBot post type and rendering it on the front end.
 */
class Popbot {
	...
	function __construct( int $post_id, int $blog_id = null ) {
		...
		if ( ! $this->post || $this->post->post_type !== static::POST_TYPE ) {
			new \Exception( __( 'Post type incorrect.', 'popbot' ) );
		}
	}

	public static function init() {
		add_action( 'init', array( static::class, 'register_post_type' ) );
		add_action( 'wp_body_open', array( static::class, 'render_all' ) );
		add_action( 'use_block_editor_for_post', array( static::class, 'use_block_editor_for_post' ), PHP_INT_MAX, 2 );

		add_shortcode( 'popbot', array( static::class, 'render_shortcode' ) );
	}

	private function refresh_post() {
		$this->set_blog();
		$this->post = get_post( $this->post_id );
		$this->reset_blog();
	}

	private function set_blog() {
		if ( is_multisite() ) {
			switch_to_blog( $this->blog_id );
		}
	}

	private function reset_blog() {
		if ( is_multisite() ) {
			restore_current_blog();
		}
	}

	private function set_meta( string $key, $value ) {
		$this->set_blog();
		$value = update_post_meta( $this->post_id, $key, $value );
		$this->reset_blog();
		return $value;
	}
	...
	static function from_uuid( string $uuid ): Popbot {
		...
		return new Popbot( $parts[1], $parts[0] );
	}
	...
	function set_template( string $template ) {
		...
		return $this->set_meta( 'template', $template );
	}

	function get_template(): string {
		return $this->get_meta( 'template' ) ?: 'native/box'; // Default template
	}
	...
	function set_trigger( $trigger ) {
		if ( is_array( $trigger ) ) {
			$trigger = wp_json_encode( $trigger );
		}

		return $this->set_meta( 'trigger', $trigger );
	}

	function get_trigger(): string {
		return $this->get_meta( 'trigger' ) ?: '{"trigger":"","threshold":""}';
	}
	...
	function set_conditions( $conditions ) {
		...
		return $this->set_meta( 'conditions', $conditions );
	}

	function get_conditions(): string {
		return $this->get_meta( 'conditions' ) ?: '[]';
	}
	...
	function set_variant_parent( $variant_parent_id ) {
		...
		if ( is_wp_error( $parent ) || $parent->post_type !== self::POST_TYPE ) {
			false;
		}

		return $this->set_meta( 'variant_parent', $variant_parent_id );
	}

	function get_variant_parent(): string {
		return $this->get_meta( 'variant_parent' ) ?: '0';
	}
	...
	function set_visibility( $visibility ) {
		...
		return $this->set_meta( 'visibility', $visibility );
	}

	function get_visibility(): string {
		return $this->get_meta( 'visibility' ) ?: '{ "visibility": "public", "start": null, "end": null }';
	}
	...
	function is_sticky(): bool {
		return boolval( $this->get_meta( 'sticky' ) );
	}

	function set_sticky( bool $sticky ) {
		return $this->set_meta( 'sticky', intval( $sticky ) );
	}
	 ...
	 */
	static function query( array $args = array() ): array {
		$default_args = array(
	...
	static function register_post_type() {
		register_post_type(
			...
			array(
				...
				'has_archive'         => false,
				'show_in_rest'        => true,
				'taxonomies'          => array( static::TAGS_TAXONOMY ),
			)
		);

		register_taxonomy(
			static::TAGS_TAXONOMY,
			static::POST_TYPE,
			array(
				'labels'            => array(
					'name'          => __( 'Tags', 'popbot' ),
