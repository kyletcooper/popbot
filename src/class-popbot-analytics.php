...
/**
 * Contains the Popbot_Analytics class.
 */

namespace popbot;

/**
 * Handles the popbot_analytics custom database table.
 */
class Popbot_Analytics {
	...
	private static function sql_where_date( $key, $start, $end, &$args ): string {
		...
		$args[] = gmdate( $timestamp_format, $start );
		$args[] = gmdate( $timestamp_format, $end );
	...
	private static function sql_select( string $columns, array $opts = array() ) {
		...
		$sql     .= ';';
        $prepared = $wpdb->prepare($sql, $args); // phpcs:ignore -- The SQL statement is constructed of literal strings and strictly validated column/table names.
		return $prepared;
	}
