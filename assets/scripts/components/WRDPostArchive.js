...
export class WRDPostArchive extends LitElement {
    static properties = {
    ...
    async getPosts() {
        var endpoint = new URL(`${window.popbot.wp.home_url}/wp-json/wp/v2/${this.post_type}`);
    ...
    async getTaxonomies() {
        // Get taxonomies
        var endpoint = new URL(`${window.popbot.wp.home_url}/wp-json/wp/v2/taxonomies`);
    ...
    render() {
        return html`
            <div class="container">
                ...
                <main class="grid">
                    ${this._posts ? repeat(this._posts, (post) => html`<wrd-bot post_id="${post.id}"></wrd-bot>`) : null}
                </main>
            </div>
        `;
    }
...
customElements.define('wrd-post-archive', WRDPostArchive);
