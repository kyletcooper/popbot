...
export class WRDTagPicker extends LitElement {
    static properties = {
        post: { type: Number },
    ...
    async _getTags() {
        this.allTags = await window.popbot.manager.fetch.api(`wp/v2/${this.taxonomy}`);
        this.postTags = await window.popbot.manager.fetch.api(`wp/v2/${this.taxonomy}`, { post: this.post });
    }
    ...
    async createTag(name) {
        await window.popbot.manager.fetch.api(`/wp/v2/${this.taxonomy}`, {
            name: name
        }, "POST");
    ...
    async _save() {
        ...
        const res = await window.popbot.manager.fetch.api(`wp/v2/${this.postType}/${this.post}`, data, "POST");
    }
...
customElements.define('wrd-tag-picker', WRDTagPicker);
